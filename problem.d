...
uint check_problem(uint id) {
   ...
   string sql_1 = "SELECT problem.pid FROM problem LEFT JOIN classol "
      ...
      ~ "WHERE classol.pid IS NULL " ~ "ORDER BY problem.pid";
   string sql_2 = "SELECT problem.pid FROM problem LEFT JOIN problemcomposer "
      ~ "ON problem.pid = problemcomposer.pid "
      ~ "WHERE problemcomposer.pid IS NULL " ~ "ORDER BY problem.pid";
   string sql_3 = "SELECT problem.sid FROM problem LEFT JOIN source "
      ~ "ON problem.sid = source.sid "
      ~ "WHERE source.sid IS NULL " ~ "ORDER BY problem.sid";
   string sql_4 = "SELECT problem.aid FROM problem LEFT JOIN award "
      ~ "ON problem.aid = award.aid "
      ~ "WHERE award.aid IS NULL " ~ "ORDER BY problem.aid";
   string sql_5 = "CREATE TEMPORARY TABLE tempb SELECT pid, eid FROM problem "
      ~ "WHERE eid != 0 " ~ "ORDER BY pid";
   string sql_5a = "SELECT tempb.pid, tempb.eid FROM tempb LEFT JOIN problem "
      ~ "ON tempb.eid = problem.pid "
      ~ "WHERE problem.pid IS NULL " ~ "ORDER BY tempb.pid";

   string sql_6 = "SELECT problem.pid, problem.eid FROM problem LEFT JOIN versions " ~ "ON problem.eid = versions.pid AND problem.pid = versions.aid " ~ "WHERE (problem.version = 'Version') AND " ~ "(problem.eid IS NOT NULL) AND " ~ "(problem.eid != 0) AND " ~ "(versions.pid IS NULL) " ~ "ORDER BY problem.pid";

   string sql_7 = "SELECT problem.pid, problem.eid FROM problem LEFT JOIN afters " ~ "ON problem.eid = afters.pid AND problem.pid = afters.aid " ~ "WHERE (problem.version = 'After') AND " ~ "(problem.eid IS NOT NULL) AND " ~ "(problem.eid != 0) AND " ~ "(afters.pid IS NULL) " ~ "ORDER BY problem.pid";
   ...
   foreach (Row row; range) {
      string mess = "PID " ~ to!string(row[0]) ~ " not in table Classol!";
      log ~= mess;
      rc++;
   }
   ...
   foreach (Row row; range) {
      string mess = "PID " ~ to!string(row[0]) ~ " not in table ProblemComposer!";
      log ~= mess;
      rc++;
   }
   ...
   foreach (Row row; range) {
      string mess = "AID " ~ to!string(row[0]) ~ " not in table Award!";
      log ~= mess;
      rc++;
   }
