...
uint check_problem_source(uint id) {
   ...
   ulong c_done;
   // dfmt off
   string sql_1	=	"SELECT problemsource.pid FROM problemsource LEFT JOIN problem "
   					~	"ON problemsource.pid = problem.pid "
   					~	"WHERE problem.pid IS NULL "
   					~	"ORDER BY problemsource.pid";
   
   string sql_2	=	"SELECT problemsource.sid FROM problemsource LEFT JOIN source "
   					~	"ON problemsource.sid = source.sid "
   					~	"WHERE source.sid IS NULL "
   					~	"ORDER BY problemsource.sid";
   
   string sql_3	=	"CREATE TEMPORARY TABLE tempc SELECT a.pid FROM problemsource AS a LEFT JOIN cants AS b "
   					~	"ON a.pid = b.pid "
   ...
   string sql_4	=	"CREATE TEMPORARY TABLE tempd SELECT a.pid FROM problemsource AS a LEFT JOIN sants AS b "
   					~	"ON a.pid = b.pid "
   ...
   foreach (Row row; range) {
      // dfmt off
      string mess	=	"Snapped PID "
      				~	to!string(row[0])
      				~	" not in tables Cants or Cabs!";
      // dfmt on
      log ~= mess;
      rc++;
   }
   ...
   foreach (Row row; range) {
      // dfmt off
      string mess	=	"Near snapped PID "
      				~	to!string(row[0])
      				~	" not in tables Sants or Sabs!";
      // dfmt on
      log ~= mess;
      rc++;
   }
